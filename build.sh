...
STARTUP_CHECKOUT_URL="${VITE_STRIPE_STARTUP_CHECKOUT_URL:-}"
...
<script src="/fca-backend-config.js"></script>
...
create_page "/terms" "Terms of Service" "<main class=\"page\"><div class=\"wrap\"><span class=\"eyebrow\">Legal</span><h1>Terms of Service</h1><section class=\"card\"><p>By using FCA you agree to our service terms. Paid plans bill through Stripe. Questions: <a href=\"mailto:info@futurecontractorsofamerica.com\">info@futurecontractorsofamerica.com</a></p></section></div></main>"

create_page "/privacy" "Privacy Policy" "<main class=\"page\"><div class=\"wrap\"><span class=\"eyebrow\">Legal</span><h1>Privacy Policy</h1><section class=\"card\"><p>FCA collects account and workspace data to operate your contractor platform. We do not sell personal information. Contact: <a href=\"mailto:info@futurecontractorsofamerica.com\">info@futurecontractorsofamerica.com</a></p></section></div></main>"
...
create_page "/intake" "Pilot Intake" "<main class=\"page\"><div class=\"wrap\"><span class=\"eyebrow\">Step 1</span><h1>Submit your company profile and start pilot onboarding</h1><section class=\"card\"><form id=\"intakeForm\"><div class=\"grid-2\"><div class=\"field\"><label for=\"plan\">Plan</label><select id=\"plan\"><option value=\"startup\">Startup</option><option value=\"starter-team\">Starter Team</option><option value=\"team\">Team</option><option value=\"operations\">Operations</option><option value=\"growth\">Growth</option><option value=\"scale-operations\">Scale Operations</option><option value=\"enterprise\">Enterprise</option><option value=\"pilot\">Pilot</option></select></div><div class=\"field\"><label for=\"company\">Company</label><input id=\"company\" required /></div><div class=\"field\"><label for=\"name\">Contact name</label><input id=\"name\" required /></div><div class=\"field\"><label for=\"email\">Email</label><input id=\"email\" type=\"email\" required /></div><div class=\"field\"><label for=\"password\">Password</label><input id=\"password\" type=\"password\" required /></div><div class=\"field\"><label for=\"confirmPassword\">Confirm password</label><input id=\"confirmPassword\" type=\"password\" required /></div></div><div id=\"intakeStatus\" class=\"status warn\" style=\"margin-top:14px\">Intake posts to Auricrux-Central backend and continues to activation.</div><div class=\"cta-row\"><button class=\"btn btn-primary\" type=\"submit\">Continue to Activation</button></div></form></section></div></main><script>const BID_API=\"https://auricrux-central.azurewebsites.net/api/bids\";const PILOT_CHECKOUT=\"${PILOT_CHECKOUT_URL}\";const STARTUP_CHECKOUT=\"${STARTUP_CHECKOUT_URL}\";const params=new URLSearchParams(location.search);const p=params.get(\"plan\");if(p)document.getElementById(\"plan\").value=p;document.getElementById(\"intakeForm\").addEventListener(\"submit\",async e=>{e.preventDefault();const pw=password.value;const cp=confirmPassword.value;if(pw!==cp){intakeStatus.className=\"status warn\";intakeStatus.textContent=\"Passwords do not match.\";return;}const rec={plan:plan.value,company:company.value,name:name.value,email:email.value.trim().toLowerCase(),password:pw};localStorage.setItem(\"fca_customer_record\",JSON.stringify(rec));intakeStatus.textContent=\"Saving intake to backend…\";try{const bidPayload={company:rec.company,projectName:rec.company+\" — \"+rec.plan,contactName:rec.name,contactEmail:rec.email,value:rec.plan===\"pilot\"?2500:rec.plan===\"startup\"?99:249,status:\"new\"};const r=await fetch(BID_API,{method:\"POST\",headers:{\"Content-Type\":\"application/json\"},body:JSON.stringify(bidPayload)});if(!r.ok)throw new Error(await r.text());intakeStatus.className=\"status success\";intakeStatus.textContent=\"Intake saved to backend. Continuing…\";}catch(err){intakeStatus.className=\"status warn\";intakeStatus.textContent=\"Local intake saved; backend sync will retry on next deploy.\";}const ref=encodeURIComponent(rec.email);if(rec.plan===\"pilot\"){location.href=PILOT_CHECKOUT+(PILOT_CHECKOUT.includes(\"?\")?\"&\":\"?\")+\"client_reference_id=\"+ref;return;}if(rec.plan===\"startup\"&&STARTUP_CHECKOUT){location.href=STARTUP_CHECKOUT+(STARTUP_CHECKOUT.includes(\"?\")?\"&\":\"?\")+\"client_reference_id=\"+ref;return;}location.href=\"/checkout/?plan=\"+encodeURIComponent(rec.plan);});</script>"
...
create_page "/login" "Client Login" '<main class="page"><div class="wrap"><span class="eyebrow">Step 3</span><h1>Sign in to your FCA workspace</h1><section class="card"><form id="loginForm"><div class="field"><label>Email</label><input id="email" type="email" /></div><div class="field"><label>Password</label><input id="password" type="password" /></div><div class="cta-row"><button class="btn btn-primary" type="submit">Sign In</button><a class="btn btn-secondary" href="/intake/">Create Account</a></div></form><div id="statusBox" class="status" style="margin-top:12px">Use your intake credentials or launch validation account.</div></section></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";const f={email:"launch.customer@futurecontractorsofamerica.com",password:"FCA-Launch-2026!"};const rec=JSON.parse(localStorage.getItem("fca_customer_record")||"null");if(rec?.email)email.value=rec.email;loginForm.addEventListener("submit",async e=>{e.preventDefault();const e1=email.value.trim().toLowerCase(),p1=password.value.trim();statusBox.textContent="Signing in…";try{const r=await fetch(CENTRAL+"/customer-login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e1,password:p1})});const j=await r.json();if(r.ok&&(j.authenticated||j.ok||j.customer)){localStorage.setItem("fca_customer_record",JSON.stringify({email:e1,password:p1,...(j.customer||{})}));statusBox.className="status success";statusBox.textContent="Login successful. Opening platform…";setTimeout(()=>location.href="/portal/platform/",350);return;}}catch(_){}if((rec&&rec.email===e1&&rec.password===p1)||(e1===f.email&&p1===f.password)){statusBox.className="status success";statusBox.textContent="Login successful. Opening platform…";setTimeout(()=>location.href="/portal/platform/",350);}else{statusBox.className="status warn";statusBox.textContent="Credentials did not validate.";}});</script>'

create_page "/portal/platform" "Platform" '<main class="page"><div class="wrap"><span class="eyebrow">Live Product Surface</span><h1>Platform Dashboard</h1><p class="lead">Your workspace for project visibility, file continuity, and activity tracking with Auricrux active.</p><div class="grid-4"><div class="card"><h3>Project</h3><p id="projectName">Loading…</p></div><div class="card"><h3>Customer</h3><p id="customerName">Loading…</p></div><div class="card"><h3>Next Action</h3><p id="nextAction">Loading…</p></div><div class="card"><h3>Workspace</h3><p id="workspaceState">Loading…</p></div></div><div class="cta-row"><a class="btn btn-primary" href="/portal/projects/">Projects</a><a class="btn btn-secondary" href="/portal/files/">Files</a><a class="btn btn-secondary" href="/portal/academy/">Academy</a><a class="btn btn-secondary" href="/portal/billing/">Billing</a><a class="btn btn-secondary" href="/portal/audit/">Audit</a><a class="btn btn-auricrux" href="/pricing/">Upgrade Plan</a></div></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";async function h(){const rec=JSON.parse(localStorage.getItem("fca_customer_record")||"null");customerName.textContent=rec?.company||rec?.email||"Launch Customer Workspace";try{const pr=await fetch(CENTRAL+"/projects").then(r=>r.json());const p=(pr.items&&pr.items[0])||(pr.projects&&pr.projects[0]);if(p){projectName.textContent=p.name||p.projectName||"Active project";nextAction.textContent=p.nextStep||p.stage||"Continue delivery";workspaceState.textContent="Live — Auricrux-Central";return;}}catch(_){}const p=await fetch("/data/live-workspace-pack.json").then(r=>r.json());projectName.textContent=rec?.company?`${rec.company} — Initial Opportunity`:p.project.name;nextAction.textContent=p.project.nextStep;workspaceState.textContent=p.workspace.status;}h();</script>'

create_page "/portal/projects" "Projects" '<main class="page"><div class="wrap"><h1>Projects</h1><section class="card"><table class="data-table"><thead><tr><th>Project</th><th>Stage</th><th>Next Step</th></tr></thead><tbody id="rows"><tr><td>Loading…</td><td>—</td><td>—</td></tr></tbody></table></section></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";async function h(){try{const pr=await fetch(CENTRAL+"/projects").then(r=>r.json());const items=pr.items||pr.projects||[];if(items.length){rows.innerHTML=items.map(p=>`<tr><td>${p.name||p.projectName}</td><td>${p.stage||"—"}</td><td>${p.nextStep||"—"}</td></tr>`).join("");return;}}catch(_){}const p=await fetch("/data/live-workspace-pack.json").then(r=>r.json());rows.innerHTML=`<tr><td>${p.project.name}</td><td>${p.project.stage}</td><td>${p.project.nextStep}</td></tr>`;}h();</script>'

create_page "/portal/files" "Files" '<main class="page"><div class="wrap"><h1>Files</h1><div id="grid" class="grid-3"></div></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";async function h(){try{const fr=await fetch(CENTRAL+"/files").then(r=>r.json());const items=fr.items||fr.files||[];if(items.length){grid.innerHTML=items.map(f=>`<article class="card"><h3>${f.name||f.fileName}</h3><p class="muted">${f.category||""} · ${f.status||"active"}</p></article>`).join("");return;}}catch(_){}const p=await fetch("/data/live-workspace-pack.json").then(r=>r.json());grid.innerHTML=p.files.map(f=>`<article class="card"><h3>${f.name}</h3><p class="muted">${f.category} · ${f.status}</p></article>`).join("");}h();</script>'

create_page "/portal/audit" "Audit" '<main class="page"><div class="wrap"><h1>Audit</h1><section class="card"><table class="data-table"><thead><tr><th>Event</th><th>Actor</th><th>Status</th></tr></thead><tbody id="rows"><tr><td>Loading…</td><td>—</td><td>—</td></tr></tbody></table></section></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";async function h(){try{const ar=await fetch(CENTRAL+"/workflow-audit").then(r=>r.json());const items=ar.items||ar.events||[];if(items.length){rows.innerHTML=items.map(a=>`<tr><td>${a.event||a.action}</td><td>${a.actor||"—"}</td><td>${a.status||"—"}</td></tr>`).join("");return;}}catch(_){}const p=await fetch("/data/live-workspace-pack.json").then(r=>r.json());rows.innerHTML=p.audit.map(a=>`<tr><td>${a.event}</td><td>${a.actor}</td><td>${a.status}</td></tr>`).join("");}h();</script>'

create_page "/academy" "FCA Academy" '<main class="page"><div class="wrap"><span class="eyebrow">LMS</span><h1>FCA Academy — Electrical Apprenticeship</h1><p class="lead">NCCER-aligned progression from Level 1 through Level 10 with live enrollment tracking on Auricrux-Central.</p><div id="programs" class="grid-3"></div><div class="cta-row"><a class="btn btn-primary" href="/login/">Enroll via Workspace</a><a class="btn btn-secondary" href="/portal/academy/">Open Academy Portal</a></div></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";async function h(){try{const r=await fetch(CENTRAL+"/academy-lms").then(x=>x.json());const progs=(r.catalog&&r.catalog.programs)||r.programs||[];if(progs.length){programs.innerHTML=progs.map(p=>`<article class="card"><h3>${p.title||p.name}</h3><p class="muted">${p.level||""} · ${p.status||"open"}</p><p>${p.summary||p.description||""}</p></article>`).join("");return;}}catch(_){}programs.innerHTML=`<article class="card"><h3>Electrical Apprenticeship L1–L10</h3><p class="muted">Catalog loading — sign in for enrollment state.</p></article>`;}h();</script>'

create_page "/portal/academy" "Academy Portal" '<main class="page"><div class="wrap"><h1>Academy Progress</h1><div id="programs" class="grid-2"></div><div id="enrollments" class="card" style="margin-top:16px"><h3>Your enrollments</h3><div id="enrollList">Loading…</div></div></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";async function h(){try{const r=await fetch(CENTRAL+"/academy-lms").then(x=>x.json());const progs=(r.catalog&&r.catalog.programs)||r.programs||[];programs.innerHTML=progs.slice(0,6).map(p=>`<article class="card"><h3>${p.title||p.name}</h3><p>${p.summary||""}</p></article>`).join("");const en=r.enrollments||[];enrollList.innerHTML=en.length?en.map(e=>`<p>${e.programId||e.title} — ${e.progress||0}%</p>`).join(""): "<p>No enrollments yet. Contact your program administrator.</p>";}catch(_){programs.innerHTML="<p>Academy catalog unavailable — retry shortly.</p>";enrollList.textContent="—";}}h();</script>'

create_page "/portal/billing" "Billing" '<main class="page"><div class="wrap"><h1>Billing</h1><section class="card"><div id="summary">Loading billing summary…</div></section><div class="cta-row"><a class="btn btn-primary" href="/pricing/">View Plans</a></div></div></main><script>const CENTRAL="https://auricrux-central.azurewebsites.net/api";async function h(){try{const r=await fetch(CENTRAL+"/billing-summary").then(x=>x.json());summary.innerHTML=`<p><strong>Plan:</strong> ${r.plan||"Startup"}</p><p><strong>Status:</strong> ${r.status||"active"}</p><p><strong>Next invoice:</strong> ${r.nextInvoice||"—"}</p>`;}catch(_){summary.textContent="Billing summary will sync when your subscription is active.";}}h();</script>'

create_page "/portal/messages" "Messages" '<main class="page"><div class="wrap"><h1>Messages</h1><section class="card"><p class="lead">Project and support threads appear here as your workspace activates.</p><div id="threads">No threads yet — submit intake or open a support case from the platform dashboard.</div></section></div></main>'

create_page "/portal/support" "Support" '<main class="page"><div class="wrap"><h1>Support</h1><section class="card"><p class="lead">FCA support is tied to your active plan and project workspace.</p><div class="cta-row"><a class="btn btn-primary" href="/contact/">Contact FCA</a><a class="btn btn-secondary" href="/portal/platform/">Back to Dashboard</a></div></section></div></main>'
...
cat > dist/deployment-status.json <<JSON
{
  ...
  "nextAction": "MNCL-007",
  "proofRoutes": ["/pricing/", "/auricrux/", "/brand-audit.json", "/deployment-status.json", "/api/stripe-webhook"],
