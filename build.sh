...
<main class="section"><div class="wrap"><span class="eyebrow eyebrow-blue">Live product proof</span><h1>FCA Platform Dashboard</h1><p class="lead">This is the current live product-proof surface for the contractor command experience. It shows visible movement beyond brochure-only pages and gives direct access to runtime proof, project-facing proof, and customer login continuity.</p><div class="grid-3"><article class="card proof"><h3>Customer path now works</h3><p class="muted">One intake flow creates a unique sign-in. Login routes here as the live product proof surface.</p></article><article class="card proof"><h3>Control-plane proof is public</h3><p class="muted">Runtime proof, deployment proof, and control-plane state are linked directly from the live domain.</p></article><article class="card proof-gold"><h3>Auricrux visible in-product</h3><p class="muted">Auricrux remains a gold-command guide across live proof surfaces so customers can see the guided lane clearly.</p></article></div><div class="cta-row"><a class="btn btn-primary" href="/auricrux/run-digest/index.json">Open Live Run Digest</a><a class="btn btn-secondary" href="/deployment-status.json">Open Deployment Status</a><a class="btn btn-secondary" href="/auricrux/live-proof/">Open Auricrux Live Proof</a></div><div class="divider"></div><div class="grid-2"><div class="card"><h2>Visible product lanes</h2><ul class="list"><li>Bid-entry continuity remains available</li><li>Customer status surface remains preserved</li><li>Machine-native state publication is live</li><li>Founder can inspect proof routes directly on domain</li></ul></div><div class="card proof-gold"><div class="logo-band" style="margin-top:0"><div class="logo-box"><img src="/favicon.svg" alt="FCA logo" style="width:58px;height:58px" /><div><strong>FCA</strong><div class="small">Contractor Command</div></div></div><div class="logo-box auricrux-box"><div class="auricrux-token">A</div><div><strong style="color:#8a6116">Auricrux</strong><div class="small">Live guidance surface</div></div></div></div><p class="muted" style="margin-top:16px">This page exists so product checks reveal real live-surface advancement rather than internal-only repo changes.</p></div></div></div></main>'
...
echo '{"status":"live-surface-repair-v3-active","shell":"FCA Contractor Command","execution":"Auricrux-Central","nextAction":"MNCL-001","proofRoutes":["/portal/platform/","/auricrux/live-proof/","/auricrux/run-digest/index.json","/deployment-status.json"]}' > dist/deployment-status.json
...
echo 'FCA live surface repair v3 active' > dist/runtime-fingerprint.txt
cat > dist/live-shell-verification.html <<'HTML'
<!doctype html><html lang="en"><head><meta charset="utf-8"><title>FCA Live Shell Verification</title></head><body><h1>FCA Live Shell Verification</h1><p>Persistent static navbar active. Home tab restored. FCA and Auricrux identity visible on-page. Unique intake-created sign-in path active. Live product proof route active at /portal/platform/.</p></body></html>
HTML
...
echo "FCA live surface repair v3 witness" > dist/commit-witness-$(date +%Y%m%d%H%M%S).txt

echo "FCA live surface repair v3 build completed"
