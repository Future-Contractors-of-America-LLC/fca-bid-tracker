...
function main() {
  ...
  const source = readJson(path.join(generatedDir, 'live-deployment-proof-provenance-validation.json'))
  const report = {
    packet,
  ...
  fs.writeFileSync(path.join(generatedDir, 'live-deployment-proof-provenance-report.json'), JSON.stringify(report, null, 2))
  fs.writeFileSync(
    path.join(generatedDir, 'live-deployment-proof-provenance-report.md'),
    [
  ...
  console.log(`Live deployment proof provenance report generated for packet ${packet}.`)
}
