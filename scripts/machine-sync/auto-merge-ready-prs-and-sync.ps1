...
$FeatureMap = @{
  ...
  179 = @("Auto-merge coordinator", "One-command repo/live sync workflow")
}
...
function Resolve-TargetPrNumbers {
  $targets = New-Object System.Collections.Generic.List[int]
  foreach ($n in $PrNumbers) {
    if (-not $targets.Contains($n)) {
      $targets.Add($n)
    }
  }

  if ($IncludePr179 -and -not $targets.Contains(179)) {
    $targets.Add(179)
  }

  if ($targets.Count -eq 0) {
    throw "No target PR numbers configured."
  }

  return @($targets)
}

function Get-PrViewSafe {
  param([int]$Number)

  $prText = Invoke-Gh -Args @("pr", "view", "$Number", "--repo", $Repo, "--json", "number,title,isDraft,state,url,mergedAt,mergeCommit,body") -AllowFailure
  if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($prText)) {
    return $null
  }

  return $prText | ConvertFrom-Json
}
...
function Merge-ReadyPr {
  ...
  $pr = Get-PrViewSafe -Number $Number
  if (-not $pr) {
    if ($Optional) {
      Write-Host "Optional PR #$Number not found; skipping." -ForegroundColor Yellow
      return $null
    }
  ...
  if ($pr.isDraft) {
    Write-Host "PR #$Number is draft. Marking ready for review..." -ForegroundColor Yellow
    Invoke-GhWrite -Args @("pr", "ready", "$Number", "--repo", $Repo) | Out-Null
    Start-Sleep -Seconds 3
  }

  Write-Host "Merging #$Number — $($pr.title)" -ForegroundColor Cyan
  $mergeArgs = @("pr", "merge", "$Number", "--repo", $Repo, "--squash", "--delete-branch")
  if ($ForceMerge) {
    $mergeArgs += "--admin"
  }
  Invoke-GhWrite -Args $mergeArgs | Out-Null
  ...
  Start-Sleep -Seconds 3
  $postMerge = Get-PrViewSafe -Number $Number
  if (-not $postMerge) {
    throw "Unable to read merged state for PR #$Number"
  }

  return [pscustomobject]@{
    number = $postMerge.number
    title = $postMerge.title
    mergedAt = $postMerge.mergedAt
    mergeCommit = $postMerge.mergeCommit
    url = $postMerge.url
    status = $postMerge.state
  }
}

function Get-StaleBranchesFromPr178 {
  $pr178 = Get-PrViewSafe -Number 178
  $body = if ($pr178 -and $pr178.PSObject.Properties.Name -contains "body" -and $pr178.body) { [string]$pr178.body } else { "" }
...
function Resolve-IssuePriority {
  param([object]$Issue)

  $labels = @($Issue.labels | ForEach-Object { $_.name.ToString().ToLowerInvariant() })
  if ($labels -match "p0|critical|blocker|sev0|sev1|urgent") { return "high" }
  if ($labels -match "p1|high|major") { return "high" }
  if ($labels -match "p2|medium|normal") { return "medium" }
  if ($labels -match "p3|low|minor|nice-to-have") { return "low" }
  return "unclassified"
}
...
$targetPrs = Resolve-TargetPrNumbers
$merged = New-Object System.Collections.Generic.List[object]
foreach ($prNumber in $targetPrs) {
  $isOptional = ($prNumber -eq 179)
  $mergedPr = Merge-ReadyPr -Number $prNumber -Optional:$isOptional
  if ($mergedPr) {
    $merged.Add($mergedPr)
  }
}
...
$openPrs = Invoke-Gh -Args @("pr", "list", "--repo", $Repo, "--state", "open", "--limit", "50", "--json", "number,title,isDraft,url") -Json
$openIssuesRaw = Invoke-Gh -Args @("issue", "list", "--repo", $Repo, "--state", "open", "--limit", "100", "--json", "number,title,labels,url") -Json

$openIssues = @()
foreach ($issue in $openIssuesRaw) {
  $priority = Resolve-IssuePriority -Issue $issue
...
$blockingIssues = @($openIssues | Where-Object { $_.priority -eq "high" })

$nextActions = @()
if ($blockingIssues.Count -gt 0) {
  $nextActions += [pscustomobject]@{ priority = "high"; action = "Resolve high-priority open issues"; count = $blockingIssues.Count }
}
...
$mainHead = if ($DryRun) { "dry-run" } else { (Invoke-Gh -Args @("api", "repos/$Repo/commits/main", "--jq", ".sha")).Trim() }
$repoTruthMatchesLiveTruth = ($deployment.conclusion -eq "success" -and $deployment.headSha -eq $mainHead)

$openPrStatus = @($openPrs | ForEach-Object {
  [pscustomobject]@{
    number = $_.number
    title = $_.title
    status = if ($_.isDraft) { "draft" } else { "ready" }
    url = $_.url
  }
})
...
$report = [pscustomobject]@{
  ...
  repository = $Repo
  mergedPullRequests = @($merged | ForEach-Object { $_ })
  featuresNowLive = @($featuresNowLive | ForEach-Object { $_ })
  staleBranchesDeleted = @($branchResult.deleted | ForEach-Object { $_ })
  staleBranchesSkipped = @($branchResult.skipped | ForEach-Object { $_ })
  ...
  liveProduction = [pscustomobject]@{
    hosts = $LiveHosts
    deployedCommitSha = $deployment.headSha
    deployedAt = $deployment.updatedAt
    featuresDeployed = @($featuresNowLive | ForEach-Object { $_ })
  }
  remainingOpenPullRequests = $openPrStatus
  openIssuesWithPriority = $openIssues
  nextActions = @($nextActions)
  repoTruthMatchesLiveTruth = $repoTruthMatchesLiveTruth
  repoTruthLiveTruthConfirmation = if ($repoTruthMatchesLiveTruth) { "Confirmed: repo truth matches live deployment head." } else { "Not yet confirmed: deployment head differs from current main head or deployment failed." }
}
