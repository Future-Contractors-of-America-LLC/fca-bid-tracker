...
fs.writeFileSync(path.join(generatedDir, 'live-deployment-ci-proof-commit-observation-validation.json'), JSON.stringify(result, null, 2))
fs.writeFileSync(
  path.join(generatedDir, 'live-deployment-ci-proof-commit-observation-validation.md'),
  [
...
if (!result.observed) {
  console.error('CI-backed live deployment proof commit not yet observed in repo-visible history.')
  process.exit(1)
}

console.log(`CI-backed live deployment proof commit observed for packet ${packet}.`)
