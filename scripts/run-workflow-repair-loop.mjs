 ...
 *  1. Observe  run live workflow simulation, capture failures
...
import {
  ...
  REPAIR_CLASSES,
} from "./lib/workflowRepairPlaybooks.mjs";
import {
  ensureDirs,
...
const MAX_ROUNDS = Math.max(1, Number.parseInt(process.env.FCA_REPAIR_LOOP_MAX_ROUNDS || "3", 10));
const RETRY_DELAY_MS = Math.max(1000, Number.parseInt(process.env.FCA_REPAIR_LOOP_RETRY_MS || "5000", 10));
const SIM_SCRIPT = path.join(root, "scripts", "simulate-contractor-workflow.mjs");
const SIM_REPORT = "docs/qc/workflow-simulation-report.json";
  ...
  for (const failure of failures.slice(0, 3)) {
    try {
      const response = await requestJson(apiBase, "/api/auricrux/actions", {
        ...
        body: {
          mode: "recommend",
          targetObjectType: "WorkflowSimulation",
          targetObjectId: failure.name,
          rationale: `Workflow repair loop failure on ${failure.name}: ${failure.detail || "unknown"}`,
...
async function main() {
  ...
  const state = readJson(root, "auricrux/system/workflow_repair_state.json", {
    runCount: 0,
  ...
  for (let round = 1; round <= MAX_ROUNDS; round += 1) {
    console.log(`\n=== Workflow repair loop round ${round}/${MAX_ROUNDS} (Observe) ===\n`);
    ...
    if (!report) {
      rounds.push({
        ...
        passed: 0,
        total: 0,
        failures: [{ name: "Simulation report", detail: "workflow-simulation-report.json missing" }],
      });
      break;
    }
    ...
    if (report.complete) {
      complete = true;
      console.log(`\n=== Round ${round}: all workflow steps passed (Review) ===\n`);
      break;
    }
  ...
  const finalSummary = complete
    ? `Workflow simulation recovered in ${rounds.length} round(s). Consecutive successes: ${state.consecutiveSuccesses}.`
    : `Workflow simulation still failing after ${rounds.length} bounded repair round(s). ${appliedRepairs.filter((r) => r.outcome.includes("queued")).length} item(s) queued. Consecutive failures: ${state.consecutiveFailures}.`;
...
main().catch((error) => {
  console.error("Workflow repair loop crashed:", error);
  process.exit(1);
});
