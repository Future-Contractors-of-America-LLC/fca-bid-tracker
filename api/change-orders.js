...
import { createChangeOrder, listChangeOrders, mutateChangeOrder } from "./finance-store.js";
...
app.http("change-orders", {
  ...
  handler: async (request) => {
    ...
    if (request.method === "GET") {
      ...
      const items = listChangeOrders(tenantId, projectId);
    ...
    try {
      const result = request.method === "POST"
        ? createChangeOrder(tenantId, body)
        : mutateChangeOrder(tenantId, body?.action, body);
      return { status: 200, jsonBody: { ok: true, ...result, backingSource: "api-finance-store" } };
    } catch (error) {
      return { status: 400, jsonBody: { ok: false, error: error?.message || "Change-order request failed." } };
    }
  },
});
