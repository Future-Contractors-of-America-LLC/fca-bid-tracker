                  ...
                  {program.courses.map((course) => (
                    <div key={course.code} style={{ border: "1px solid #e5e7eb", borderRadius: 12, padding: 14, background: "#f8fafc" }}>
                      ...
                      <div style={{ display: "grid", gap: 12 }}>
                        ...
                        <div style={detailCardStyle}><div style={{ fontWeight: 700, marginBottom: 6 }}>Remediation paths</div><div style={{ color: "#334155", lineHeight: 1.8 }}>{course.remediationPaths.map((item) => <div key={item}>• {item}</div>)}</div></div>
                      </div>
                    </div>
                  ))}
                </div>
