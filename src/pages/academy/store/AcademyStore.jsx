...
export default function AcademyStore() {
  ...
  const [catalog, setCatalog] = useState({ courses: [], pathways: [], lanes: [], stripeConfigured: false });
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError("");
      try {
        const payload = await fetchAcademyCommerceCatalog({ lane: lane || undefined });
        if (!active) return;
        setCatalog(payload);
      } catch (loadError) {
        if (!active) return;
  ...
  const visibleCourses = useMemo(() => catalog.courses || [], [catalog.courses]);
  const visiblePathways = useMemo(() => catalog.pathways || [], [catalog.pathways]);
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 20px 48px" }}>
        ...
        {!loading && !error && tab === "courses" ? (
          <div style={{ display: "grid", gap: 16, gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))" }}>
            {visibleCourses.slice(0, 60).map((course) => (
