...
function buildBriefingMutation(file, visibleProject) {
  return {
    ...
    briefingSummary: `Auricrux plan/spec briefing for ${file.name} on ${visibleProject.id} — MEP plenum and finish schedule gaps flagged before EST-1.`,
    briefingKeyFacts: [
      `${file.category || "Document"} registered on ${visibleProject.id} file spine with FCAS/FCAM lineage.`,
      `Sheet manifest linked to Package A-117 — reconcile mechanical quote before estimate release.`,
    ],
    briefingDetectedGaps: [
      "MEP plenum conflict on sheet A-201 — confirm ceiling height before takeoff quantities import to EST-1.",
    ],
