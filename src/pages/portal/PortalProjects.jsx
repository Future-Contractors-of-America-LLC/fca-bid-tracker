...
export default function PortalProjects() {
  ...
  const { projects, advanceProjectStage, clearPermitBlocker, setActiveProject } = useProjectWorkspace();
  ...
  return (
    ...
    >
      ...
      <CommercialContinuityFeed title="Project commercial continuity feed" detail="Recent project-stage changes, permit-path repairs, execution-to-closeout mutations, and active-project rebinding remain visible here so delivery actions stay tied to revenue and rollout continuity." />
      <AutomationRecoveryFeed title="Project automation feed" detail="Recent Auricrux project repairs, stage transitions, and workspace project selection events remain visible across routes so execution-state changes are durable." />
      ...
      <div style={{ ...cardStyle, marginBottom: 16, background: "linear-gradient(135deg, #eff6ff 0%, #ffffff 100%)", border: "1px solid #dbe3ef" }}>
        ...
        <div style={{ color: "#475569", lineHeight: 1.8 }}>
          ...
          <div><strong>Active project ID:</strong> {state.meta.activeProjectId || state.project.id}</div>
        </div>
      </div>
        ...
        {projects.map((project) => {
          ...
          return (
            <div
              ...
              style={{
                ...cardStyle,
                border: isActive ? "1px solid #2563eb" : cardStyle.border,
                background: isActive ? "linear-gradient(135deg, #eff6ff 0%, #ffffff 100%)" : cardStyle.background,
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                ...
                <div style={{ minWidth: 220, color: "#0f172a", lineHeight: 1.7 }}>
                  ...
                  <div><strong>Active root:</strong> {isActive ? "Yes" : "No"}</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 14 }}>
                <button
                  ...
                  onClick={() => setActiveProject(project.id, `Project ${project.id} selected as the active workspace root.`)}
                >
