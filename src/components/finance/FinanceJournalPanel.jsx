...
export default function FinanceJournalPanel({ items, onPost, busy }) {
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <div style={card}>
        <div style={{ fontWeight: 800, marginBottom: 8 }}>Post manual journal entry</div>
        <p style={{ color: "#64748b", marginTop: 0, lineHeight: 1.6 }}>Governed double-entry posting — debits must equal credits.</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
          <input style={input} placeholder="Memo" value={draft.memo} onChange={(e) => setDraft({ ...draft, memo: e.target.value })} />
          <input style={input} placeholder="Debit account" value={draft.debitAccount} onChange={(e) => setDraft({ ...draft, debitAccount: e.target.value })} />
          <input style={input} placeholder="Debit amount" value={draft.debitAmount} onChange={(e) => setDraft({ ...draft, debitAmount: e.target.value, creditAmount: e.target.value })} />
          <input style={input} placeholder="Credit account" value={draft.creditAccount} onChange={(e) => setDraft({ ...draft, creditAccount: e.target.value })} />
        </div>
        <button type="button" style={{ ...btn(true), marginTop: 12 }} disabled={busy} onClick={handlePost}>
          Post to GL
        </button>
      </div>

      <div style={card}>
        <div style={{ fontWeight: 800, marginBottom: 12 }}>Journal register</div>
        <div style={{ display: "grid", gap: 12 }}>
          ...
          {!(items || []).length ? <div style={{ color: "#64748b" }}>No journal entries yet. Bills, expenses, invoices, and payments post automatically.</div> : null}
        </div>
      </div>
    </div>
  );
