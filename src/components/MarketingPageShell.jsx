...
export default function MarketingPageShell({
  ...
  return (
    <div style={{ ...pageShellStyle, background: "#ffffff", minHeight: "100vh", paddingTop: 0, maxWidth: "none", margin: 0, width: "100%" }}>
      ...
      <section
        style={{
          background: "linear-gradient(165deg, #0c2340 0%, #1e3a5f 42%, #0c2340 100%)",
      ...
      >
        <div
          style={{
            ...
            margin: "0 auto",
            display: "grid",
            gridTemplateColumns: illustrationKey ? "repeat(auto-fit, minmax(min(100%, 320px), 1fr))" : "1fr",
            gap: 28,
            alignItems: "center",
          }}
        >
          <div>
            {eyebrow ? (
              <div style={{ color: "#c4a052", fontWeight: 700, marginBottom: 10, letterSpacing: "0.1em", textTransform: "uppercase", fontSize: 12 }}>
                {eyebrow}
              </div>
            ) : null}
            <h1 style={{ marginTop: 0, fontSize: "clamp(1.85rem, 3.5vw, 2.75rem)", lineHeight: 1.15, fontWeight: 800, maxWidth: 720, color: "#f8fafc" }}>
              {title}
            </h1>
            {subtitle ? (
          ...
          {illustrationKey ? (
            <div style={{ filter: "drop-shadow(0 18px 40px rgba(0,0,0,0.28))" }}>
              <ProductIllustration variant={illustrationKey} compact />
            </div>
          ) : null}
        </div>
      </section>
